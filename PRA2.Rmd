...
process_data <- function(data) {
  data %>%
    mutate(Features = ifelse(is.na(Features) | Features == "", "NoFeature", Features)) %>%  # Substituir valors buits amb "NoFeature"
    separate_rows(Features, sep = ", ") %>%
    mutate(value = 1) %>%
    pivot_wider(names_from = Features, values_from = value, values_fill = list(value = 0)) %>%
    select(-Parking)
}
...
>
> A continuació farem una comprovació de la igualtat de variàncies entre les propietats cèntriques i no cèntriques.
> En aquest cas com les dades no compleixen la condició de normalitat, utilitzarem el test de Fligner-Killeen.
...
hab_parroquia_df$Informacio <- tolower(sub(".*?\\.\\s*(.*)\\.$", "\\1", hab_parroquia_df$Informacio))

zone_counts <- as.data.frame(table(preus_propietats_df$Zone))
colnames(zone_counts) <- c("Zone", "Registres")  


zone_counts$Mod_zone <- sapply(strsplit(as.character(zone_counts$Zone), "-"), function(x) x[1])

hab_parroquia_df$Mod_zone <- sapply(strsplit(as.character(hab_parroquia_df$Informacio), " "), function(x) x[1])

prop_by_zone <- merge(hab_parroquia_df, zone_counts, by.x = "Mod_zone", by.y = "Mod_zone", all.x = TRUE)[, c("Zone", "Registres", "Valor")]
...
t_test_result_venda <- t.test(habitatges_venda_centrics$Price, habitatges_venda_no_centrics$Price, alternative = "greater")
...
t_test_result_lloguer <- t.test(habitatges_lloguer_centrics$Price, habitatges_lloguer_no_centrics$Price, alternative = "greater")
...
```{r message=FALSE, warning=FALSE}
# Obtenir els ingressos anuals per unitat de consum
ingressos_anuals_per_unitat <- ingressos_medians_df$Valor

# Calcular el lloguer mitjà anual per parròquia
lloguer_mitja_anual_per_parroquia <- lloguer_data %>%
  group_by(Zone) %>%
  summarise(Lloguer_Mitja = mean(Price, na.rm = TRUE) * 12)  # Multiplicar per 12 per obtenir lloguer anual

# Crear les dades per al gràfic
dades_grafic <- merge(lloguer_mitja_anual_per_parroquia, data.frame(Zone = unique(lloguer_data$Zone), Ingressos_Anuals = ingressos_anuals_per_unitat), by = "Zone")

# Crear el gràfic
ggplot(dades_grafic, aes(x = Zone)) +
  geom_bar(aes(y = Lloguer_Mitja, fill = "Cost Anual Lloguer"), stat = "identity") +
  geom_hline(aes(yintercept = Ingressos_Anuals), color = "red", linetype = "dashed", size = 1) +
  geom_text(aes(x = Inf, y = Ingressos_Anuals, label = "Ingressos Anuals"), color = "red", vjust = -0.5, hjust=1.) +
  geom_hline(aes(yintercept = Ingressos_Anuals * 0.35), color = "darkviolet", linetype = "dashed", size = 1) +
  geom_text(aes(x = Inf, y = Ingressos_Anuals * 0.35, label = "35% Ingressos"), color = "darkviolet", vjust = -0.5, hjust=1.) +
  labs(title = "Lloguer Mitjà Anual vs Ingressos Medians Anuals per Unitat de Consum",
       x = "Parròquia",
       y = "Preu anual mig del lloguer (€)") +
  theme_minimal() +
...
> Podem fer un anàlisi per veure la relació entre el salari mig i el preu del lloguer.
> Segons les directrius estàndard d'habitatge i finances personals és recomanable que el cost de l'habitatge no superi el 35% dels ingressos totals.
> Aquesta anàlisi calcula la mediana dels preus anuals de lloguer per parròquia i determina quin percentatge de lloguers supera aquest llindar del 35% en relació als ingressos anuals per unitat de consum.
...
```{r message=FALSE, warning=FALSE}
# Calcular la mediana de preus anuals per parròquia
mediana_lloguer_anual_per_parroquia <- lloguer_data %>%
  group_by(Zone) %>%
  summarise(Mediana_Lloguer_Anual = median(Price) * 12)

# Calcular el percentatge d'habitatges per parròquia on el lloguer anual supera el 35% dels ingressos anuals
percentatge_superior_35_per_parroquia <- lloguer_data %>%
  group_by(Zone) %>%
  summarise(Percentatge_Superior_35 = sum((Price * 12) > (ingressos_anuals_per_unitat * 0.35)) / n() * 100)

# Calcular el nombre de salaris recomanats necessaris per cobrir el lloguer anual
salaris_necessaris_per_parroquia <- lloguer_data %>%
  group_by(Zone) %>%
  summarise(Salaris_Necessaris = mean((Price * 12) / (ingressos_anuals_per_unitat*0.35)))

# Combinar les dades en una nova taula
dades_grafic %>%
  select(Zone, Lloguer_Mitja) %>%
  inner_join(mediana_lloguer_anual_per_parroquia, by = "Zone") %>%
  inner_join(percentatge_superior_35_per_parroquia, by = "Zone") %>%
  inner_join(salaris_necessaris_per_parroquia, by = "Zone") %>%
  rename(Parròquia = Zone, 
         Mitjana_Lloguer_Anuals = Lloguer_Mitja, 
         Mediana_Lloguer_Anuals = Mediana_Lloguer_Anual,
         Percentatge_Superior_35 = Percentatge_Superior_35, 
         Salaris_Necessaris = Salaris_Necessaris)

```

> De la taula de resultat podem extreure el següent:
>
> -   Andorra-la-Vella: Amb una mitjana de lloguer anual de 37,568.20€ i una mediana de 30,000€, tots els lloguers (100%) superen el 35% dels ingressos anuals per unitat de consum.
>     Es recomanen aproximadament 4.48 salaris per pagar el lloguer, indicant una alta càrrega de lloguer.
>
> -   Canillo: Presenta la mitjana de lloguer més alta (53,272.94€) amb una mediana també elevada (54,000€).
>     Tot com Andorra-la-Vella, tots els lloguers superen el 35% dels ingressos, recomanant-se 6.35 salaris, el que implica una gran pressió financera pel lloguer.
>
> -   Encamp: Amb una mitjana de 26,314.29€ i una mediana similar, el 85.71% dels lloguers superen el llindar del 35% dels ingressos.
>     Es requeririen 3.14 salaris per pagar el lloguer, una pressió financera més moderada comparada amb altres zones.
>
> -   Escaldes-Engordany: El lloguer mitjà és de 48,704.35€, amb un 95.65% dels lloguers superant el 35% dels ingressos.
>     Es recomanen gairebé 5.8 salaris, indicant una alta càrrega de lloguer.
>
> -   La Massana: Amb una mitjana de lloguer de 39,900€ i una mediana de 34,800€, tots els lloguers superen el 35% dels ingressos, recomanant-se prop de 4.75 salaris.
>
> -   Ordino: Presenta una situació similar amb una mitjana de lloguer de 36,400€, tots els lloguers superant el 35% dels ingressos, requerint 4.34 salaris.
>
> -   Sant Julià de Lòria: Amb la mitjana més baixa (21,970€) i una mediana de 20,340€, un 95.83% dels lloguers superen el 35% dels ingressos, però es requereixen només 2.62 salaris.
>
> En conclusió, aquestes dades mostren que el cost del lloguer a Andorra suposa una càrrega financera significativa en comparació amb els ingressos anuals per unitat de consum, especialment a Canillo i Escaldes-Engordany, on es requereixen més de cinc salaris per pagar el lloguer i tenir una economia saludable.
> Això pot indicar una falta d'accessibilitat als habitatges per a moltes famílies, especialment en aquestes zones.
>
> Per a fer una regressió entre el preu i la resta de variables primerament hem de convertir la :

```{r}
colnames(lloguer_data)
```

>Per a intentar comprende com característiques com l'àrea, el nombre d'habitacions, disponibilitat d'aparcament i ubicació afecten als preus dels habitatges, farem un model de regressió lineal per els pisos en venda i un altra pels pisos en lloguer. Per a fer-ho el primer pas és convertir la variable zone en una variable dicotòmica:
...
> L'estudi detallat del mercat immobiliari andorrà revela una complexa realitat que es manifesta tant en la disponibilitat d\'habitatges com en els preus de venda i lloguer d\'aquests.
>
> Si bé es tracta d\'una tendència general, s\'ha observat que els preus de venda varien significativament
> entre parròquies (al contrari que els de lloguer, que semblen ser més uniformes).
> Per contra, no s\'ha observat diferències significatives pel que fa a la superfície entre les parròquies més cèntriques i les que no ho són tant; si bé la seva distribució en nombre d\'habitacions, sobretot per les propietats en venda, sí que varia significativament.
>
> S'HA DE CONTINUAR A PARTIR DE LES CORRELACIONS!
>
>
>
>
>
> Els resultats de la regressió suggereixen que les variables associades a l'àrea de l'habitatge i el nombre d'habitacions són importants per predir els preus de venda i lloguer. A més, la proximitat al centre també influeix positivament en els preus de lloguer, així com que l'habitatge es trobi en la zona de Canillo. En canvi en el cas de venda, les parròquies amb més influència en el preu són Ordino i Canillo.

## Generació del fitxer

```{r}
write.csv(processed_data, file = "dataset/preus_propietats_andorra_dades_finals.csv", row.names = FALSE)
```
