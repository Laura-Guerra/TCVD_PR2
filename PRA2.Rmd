...
ggplot(prop_by_zone, aes(x = Zone)) +
  geom_bar(aes(y = Registres/Valor, fill = "Count/Valor"), stat = "identity", position = "fill") +
  labs(title = "Proporción de Count sobre Valor por Zona",
       y = "Proporción",
       x = "Zona") +
  theme_bw() +
...
> Alhora, es preten estudiar les distribucions de preus de lloguer i venda de les dues zones definides, i
> comparar els valors mitjans de les dos grups mitjançant un contrast d'hipòtesi per determinar si el preu a la zona cèntrica és superior.
...
ggplot(dades_grafic, aes(x = Zone)) +
  ...
  geom_hline(aes(yintercept = Ingressos_Anuals * 0.35), color = "darkgreen", linetype = "dashed", size = 1) +
  geom_text(aes(x = Inf, y = Ingressos_Anuals * 0.35, label = "35% Ingressos"), color = "darkgreen", vjust = -0.5, hjust=1.) +
...
process_data <- function(data) {
  data %>%
    ...
    select(-URL, -Timestamp, -Parking, -Agency) # Eliminem columnes innecessaries
}

processed_lloguer_data <- process_data(lloguer_data)
processed_venda_data <- process_data(venda_data)

colnames(processed_lloguer_data)
```
