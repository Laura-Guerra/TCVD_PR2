...
plot_quartiles <- function(variable) {
  q1 <- quantile(preus_propietats_df[[variable]], 0.25, na.rm = TRUE)
  q3 <- quantile(preus_propietats_df[[variable]], 0.75, na.rm = TRUE)
  
  plt <- ggplot(preus_propietats_df, aes_string(x = "Type", y = variable, fill = "Type")) +
    geom_boxplot() +
    labs(title = paste("Distribució de la variable", variable, "segons lloguer/venda"),
         y = variable,
         subtitle = paste("Q1:", q1, ", Q3:", q3)) +  
    theme_minimal()
...
for (variable in c("Price", "Area", "Bedrooms")) {
  plot_and_print_quartiles(variable)
}
```
...
preus_propietats_df <- preus_propietats_df[preus_propietats_df$Area > 0, ]
```
