...
normality_check <- function(variable) {
cat("Shapiro-Wilk Test per la variable", variable, ":\n")
    shapiro_test <- shapiro.test(preus_propietats_df[[variable]])
    print(shapiro_test)
    
    par(mfrow = c(1, 2))  
    
    qqnorm(preus_propietats_df[[variable]])
    qqline(preus_propietats_df[[variable]])
    
    hist(preus_propietats_df[[variable]], main = paste("Histograma de", variable), col = "lightblue", border = "black")
...
for (variable in c("Price", "Area", "Bedrooms")) {
  normality_check(variable)
}


```

**NO TINC MOLT CLARA LA PART DE LA HOMOGENEÏTAT DE LA VARIÀNCIA**
