...
# Calcular el nombre de salaris necessaris per cobrir el lloguer anual
salaris_necessaris_per_parroquia <- lloguer_data %>%
  group_by(Zone) %>%
  summarise(Salaris_Necessaris = mean((Price * 12) / ingressos_anuals_per_unitat))

# Combinar les dades en una nova taula
taula_resultats <- dades_grafic %>%
